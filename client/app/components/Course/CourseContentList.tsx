  ...
  return (
    <div className={`mt-[15px] w-full ${!props.isDemo && 'ml-[-30px] sticky top-24 left-0 z-30'}`}>
        {videoSections.map((section: string, sectionIndex: number) => {
            ...
            // start index of videos within current section
            const sectionStartIndex: number = totalCount;

            // update total count of videos
            totalCount +=  sectionVideoCount;

            const sectionContentHours: number = sectionVideoLength / 60;

            return (
               <div className={`${!props.isDemo && 'border-b dark:border-[#ffffff8e]  border-gray-300 pb-2'}`} key={section}> 
               <div className="w-full flex">
                    {/* render video section */}
                    <div className='w-full flex justify-between items-center'>
                        <h2 className='text-[20px] 800:text-[22px] font-semibold text-gray-900 dark:text-white'> {section} </h2> 

                            <button className='mr-3 cursor-pointer text-black dark:text-white'
                            ...
                            <h5 className='mt-1 text-[14px] text-gray-600 dark:text-gray-400'>
                                ...
                                isSectionVisible && (
                                    <div className="w-full">
                                        {sectionVideos.map((item: any, index: number) => {
                                            ...
                                            return (
                                                <div className={`w-full ${ videoIndex === props.activeVideo ? "dark:bg-slate-800 bg-slate-200  " : ""
